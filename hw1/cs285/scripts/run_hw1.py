...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, params['eval_batch_size'],
                                                                   params['ep_len'])
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
            ...
            random_index = np.random.permutation(len(replay_buffer.obs))
            random_index_clipped = random_index[:batch_size]
            ob_batch, ac_batch = replay_buffer.obs[random_index_clipped], replay_buffer.acs[random_index_clipped]
