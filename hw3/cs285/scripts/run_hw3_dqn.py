...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(np.array(action), reward, next_observation, done)
        else:
            # We're using the regular replay buffer
            replay_buffer.insert(observation, np.array(action), reward, next_observation, done)
        ...
        if step >= config["learning_starts"]:
            ...
            update_info = agent.update(obs=batch["observations"], action=batch["actions"], reward=batch["rewards"],
